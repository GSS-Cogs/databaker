Feature: Define a dimension as a range of values from a cell reference but not including the blank cells.
  ...
  Scenario Outline: Define year from a range of cell references ignoring any blank cells.
    Given we load a file named <File Name>
    And select the sheet "Sheet3"
    And we define cell selections as
      | key             | value                                         |  
      | year            | tab.excel_ref("A11:A250").is_not_blank()      |
